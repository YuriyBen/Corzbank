...
export class HeaderComponent implements OnInit {
  ...
  isUserAuthenticated() {
    const accessToken = JSON.parse(this.storageService.getItem(StorageTypeEnum.LocalStorage, Constants.AccessTokenKey) as string);
    ...
    if (accessToken && !this.jwtHelper.isTokenExpired(accessToken)) {
      ...
      if (refreshToken && !this.jwtHelper.isTokenExpired(refreshToken)) {
        this.authenticationService.refreshTokensForUser(refreshToken).subscribe((response: any) => {
