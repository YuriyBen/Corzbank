...
export class ResentVerificationComponent implements OnInit {
  ...
  resend() {
    ...
    this.authenticationService.resendVerification(resendVerification).subscribe(data=>{
      if (data)
      this.dialog.open(ConfirmEmailComponent, { disableClose: true, data: resendVerification.email });
    else {
